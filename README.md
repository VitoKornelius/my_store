...
У `packages/api/src/scripts` є кілька скриптів наповнення:

| Команда | Що вона додає |
| --- | --- |
| `bun run seed` | базові налаштування та стартові товари |
| `bun run seed:marketplace` | товари для перших чотирьох продавців |
| `bun run seed:catalog` | великий каталог для інших восьми продавців |
| `bun run seed:more-products` | ще по кілька товарів усім 12 продавцям |

Запускати їх потрібно з папки `packages/api`.

```powershell
cd packages/api
bun run seed:marketplace
bun run seed:catalog
bun run seed:more-products
```

Важливо: marketplace-скрипти **не створюють продавців**. Вони лише знаходять уже зареєстровані seller-акаунти за email і додають їм товари.

Скрипти перевіряють товари за `handle`, тому їх можна запускати повторно без створення дублікатів.
